...
public class District implements IDistrict {
    ...
    public void fillDistrict(){
        ...
        Minions.d("MAX size: " + max.size());
        for(Parcel parcel : max){
//        	if(parcel.getSchematicType() != SchematicType.FILLER) {
	            grid.fillParcels(parcel.getChunkX(),parcel.getChunkZ(),parcel);
	            Minions.d("Parcel: " + parcel.toString());
//        	}
        }
...
//                    p = placeRandomBuildForSure(new Cartesian2D(x,y),new Cartesian2D(2,2),new HashSet<ClipboardParcel>());
                    if(p==null){
                        Minions.d("Can't find any clipboard for this spot? Seriously?");
                    } else {
                        Minions.d("placeRandomBuildForSure: " + p.toString());
                        grid.setParcel(x,y,p);
                    }
                }else {
                	Minions.d("Parcel WAS filled by district! " + p.toString());
                }
            }
        }
    }
    ...
    private ClipboardParcel placeRandomBuildForSure(Cartesian2D base,Cartesian2D size,Set<ClipboardParcel> previous){
        ...
        if(p!=null) {
        	Minions.d("PRBFS BUILD PARCEL != NULL");
        	return p;
        }
        p = placeRandomBuild(base,size,previous,SchematicType.FILLER);
        if(p!=null) {
        	Minions.d("PRBFS FILLER PARCEL != NULL");
        	return p;
        }
    	Minions.d("PRBFS BUILD AND FILLER PARCEL == NULL");
...
//        List<Clipboard> clips = grid.getClipboardProvider().getFit(size, context,SchematicType.BUILD, direction);
        if(clips.size()==0){ // fallback to no context?
            ...
            Minions.d("PRBFS found filler for context '%s', size '%s' and direction '%s'",context,size,direction);
        }
        return new ClipboardParcel(grid, base, size, clips.get(grid.getRandom().getRandomInt(clips.size())), context,SchematicType.BUILD, direction);
    }
    ...
    private int nextRecPartition(Set<ClipboardParcel> parcels,Cartesian2D base, Cartesian2D size){
        ...
        Minions.d("NRP started");
        if(size.X<=1 && size.Y<=1){ // are we already at smallest possible size?
            Minions.d("NRP Smallest size possible");
            if((parcel= placeRandomBuildForSure(base, size,parcels))!=null){
                Minions.d("NRP Smallest size possible : PRBFS != null");
                parcels.add(parcel);
                return scoreParcel(parcel,size);
            }else {
                Minions.d("NRP Found absolutely no schems for context '%s' and size '%s' ",context,size);
        ...
        Minions.d("NRP Trying to place schematic");
        if((parcel= placeRandomBuild(base, size,parcels))!=null){
            Minions.d("NRP PRB != null");
            parcels.add(parcel);
            return scoreParcel(parcel, size);
        }
