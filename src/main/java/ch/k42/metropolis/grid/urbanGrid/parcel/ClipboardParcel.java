...
public class ClipboardParcel extends Parcel {
    ...
    public int hashCode() {
    	if(clipboard == null) {
    		System.out.println("CP CP IS NULL");
    	}
    	if(clipboard.getGroupId() == null) {
    		System.out.println("CP GROUPID IS NULL");
    	}
    ...
    public void populate(MetropolisGenerator generator, Chunk chunk) {
        if (chunk.getX() == (chunkX) && chunk.getZ() == (chunkZ)) {
            if(clipboard==null) {
            	Minions.d("ClipboardParcel is null, and will not be placed! (" + chunk.getX() + ";" + chunk.getZ() + ")");
            	return;
            }
    ...
    public void postPopulate(MetropolisGenerator generator, Chunk chunk) {
        ...
        try {
        	...
        	Minions.d("CLIPBOARD PARCEL DECAY DIDNT WORK HERES SOME INFO");
        	Minions.d("Generator null? : " + (generator == null));
        	Minions.d("Generator decay provider null? : " + (generator.getDecayProvider() == null));
        	Minions.d("clipboard group ID : " + clipboard.getGroupId());
        	Minions.d("clipboard hashCode null? : " + clipboard.hashCode());
