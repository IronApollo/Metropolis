...
public class EBeanServerUtil {
    public static EbeanServer build(final MetropolisPlugin javaPlugin) {
        javaPlugin.getLogger().info("Creating EbeanServer for plugin with name: " + javaPlugin.getDescription().getName());
        ...
        try {
            int i = ebeanServer.find(classes.get(0)).findRowCount();
            Minions.i("Database exists. Row count: " + i);
        } catch (final PersistenceException e) {
            createDdl(ebeanServer);
            Minions.i("Creating DDL.");
        }
